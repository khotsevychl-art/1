...
let currentPage = 1;
let currentItems: NoteDto[] = [];
let currentTotal = 0;
let coursesState: CourseDto[] = [];
...
function getQuery() {
  return {
    ...
    sortBy: elements.sortBySelect.value as "title" | "createdAt" | "courseId",
    sortDir: elements.sortDirSelect.value as "asc" | "desc",
    page: currentPage,
    pageSize: Number(elements.pageSizeSelect.value),
  };
}

function validate(dto: CreateNoteDto) {
  clearErrors();
  let ok = true;
...
async function loadList(force = false) {
  ...
  try {
    ...
    if (currentItems.length === 0) renderListStatus("empty");
    else renderListStatus("success");
  } catch (e) {
...
function resetUserForm() {
  ...
  elements.createUserBtn.textContent = "Створити користувача";
  elements.cancelUserEditBtn.hidden = true;
  clearErrors();
}

function readForm(): CreateNoteDto {
  return {
    ...
    note: (document.getElementById("noteText") as HTMLTextAreaElement).value.trim(),
  };
}
...
elements.usersList.addEventListener("click", (e) => {
  ...
  if (!id || !target.classList.contains("edit-user-btn")) return;
  ...
  elements.createUserBtn.textContent = "Зберегти користувача";
  elements.cancelUserEditBtn.hidden = false;
...
elements.notesTableBody.addEventListener("click", async (e) => {
  ...
  if (target.classList.contains("edit-btn")) {
    ...
    (document.getElementById("titleInput") as HTMLInputElement).value = note.title;
    (document.getElementById("noteText") as HTMLTextAreaElement).value = note.note;
